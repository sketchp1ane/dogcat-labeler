...
node scripts/init-database.js      # 初始化数据库和默认用户
...
通过运行数据生成脚本，系统可以包含丰富的测试数据：
- **14个不同角色用户**（通过init-database.js创建）
- **大量模拟标注任务**（通过generate-complex-data.js生成）
- **完整的标注和审核记录**
- **60天历史数据**（真实的工作日/周末分布）
...
│   ├── config/               # 配置文件 (数据库等)
│   ├── scripts/              # 数据库管理脚本
│   │   ├── init-database.js     # 数据库初始化和用户创建
│   │   ├── generate-complex-data.js  # 复杂测试数据生成
│   │   ├── check-database.js         # 数据库状态检查
│   │   ├── analyze-annotator-performance.js  # 性能分析
│   │   └── README.md               # 脚本使用说明
│   ├── uploads/              # 文件上传目录 (git ignored)
│   ├── public/               # 静态文件
│   ├── app.js                # Express应用配置
│   └── package.json          # 后端依赖配置
├── start.sh                   # 一键启动脚本
...
> 💡 **脚本使用顺序**: 先运行 `init-database.js` 创建数据库和用户，再运行 `generate-complex-data.js` 生成测试数据
